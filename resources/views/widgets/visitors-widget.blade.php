    ...
    <div @class(['filament-stats-overview-widget-card relative rounded-xl bg-white p-6 shadow-sm ring-1 ring-gray-950/5 dark:bg-gray-800 dark:ring-white/20'])>
        <div @class(['space-y-2'])>
            ...
            <div wire:init='init'>
                ...
                <div class="flex items-center space-x-1 text-sm font-medium text-custom-600 rtl:space-x-reverse" style="{{ \Filament\Support\get_color_css_variables($data['color'] ?? 'gray', shades: [600]) }}">
                    <span>{{ $data['description'] }}</span>

                    <x-filament::icon :name="$data['icon']" alias="widgets::stats-overview.card.description" size="h-4 w-4" />
                </div>
                @else
